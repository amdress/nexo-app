...
export const dailyService = {
  ...
  async createDaily(payload: DailyCreateDto): Promise<DailyItem> {
    try {
      ...
      if (payload.selectedStaffIds && payload.selectedStaffIds.length > 0) {
        const staff: DailyStaffEntity[] = payload.selectedStaffIds.map(
          (staffId) => ({
        ...
        console.log(
          `[DAILY_SERVICE] Alocados ${staff.length} funcionários selecionados.`,
        );
      }
      ...
      console.error(
        "[DAILY_SERVICE] Erro ao salvar jornada e tabela pivô:",
        error,
      );
      throw new Error(
        error?.message || "Erro interno ao persistir a jornada de trabalho",
      );
    }
  },
  ...
  async getDailyWithStaff(dailyId: string): Promise<DailyWithStaffUI | null> {
    try {
      ...
      console.error(
        "[DAILY_SERVICE] Erro ao buscar jornada com staff mapeada:",
        error,
      );
      throw new Error(
        error?.message || "Não foi possível carregar os dados da jornada.",
      );
    }
  },
  ...
  async addEmergencyStaff(dailyId: string, staffId: string): Promise<void> {
    try {
      ...
      console.log(
        `[DAILY_SERVICE] Staff de emergência ${staffId} adicionado com sucesso à jornada ${dailyId}`,
      );
    } catch (error: any) {
      console.error(
        "[DAILY_SERVICE] Erro ao adicionar staff de emergência:",
        error,
      );
      throw new Error(
        error?.message || "Não foi possível adicionar o funcionário.",
      );
    }
  },
  ...
  async updateDailyStatus(
    ...
    try {
      ...
      if (newStatus !== daily.status) {
        ...
        console.log(
          `[DAILY_SERVICE : STATUS DE JORNADA] Status da jornada ${dailyId} atualizado para: ${newStatus}`,
        );
      }
  ...
  async updateStaffStatus(
    ...
    try {
      ...
      console.log(
        `[DAILY_SERVICE : UPDATE ] Atualizado — daily: ${dailyId}, staff: ${staffId}`,
        dataToUpdate,
      );
    } catch (error: any) {
      console.error(`[DAILY_SERVICE : UPDATE] Erro ao atualizar:`, error);
  ...
  async saveStaffSignature(
    ...
    try {
      ...
      const signatureUri = await saveSignatureFile(
        dailyId,
        staffId,
        signatureBase64,
      );
      ...
      console.log(
        `[DAILY_SERVICE : Assinatura Salva ] Assinatura salva — daily: ${dailyId}, staff: ${staffId}`,
      );
    } catch (error: any) {
      console.error(
        "[DAILY_SERVICE: assiatura ] Erro ao salvar assinatura do staff:",
        error,
      );
      throw new Error(
        error?.message || "Não foi possível salvar a assinatura.",
      );
    }
  },
};
